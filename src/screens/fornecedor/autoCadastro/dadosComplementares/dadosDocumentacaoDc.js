...
import { FUNCIONALIDADE, TAB_DADOS_COMPLEMENTAR } from '@/utils/constants';
...
export default function DadosDocumentacaoDc({ key, documentos_db, formulario, tab }) {
	...
	const buscaDcPorFuncionalidade = () => {
		...
		TipoDocumentoService.findByFuncionalidade(FUNCIONALIDADE.CADASTRO_PESSOA_JURIDICA)
			.then(response => {
				...
				listaDocumentacaoDcPreencher(response.data.TipoDocumento_list);
				dispatch(LoaderCreators.disableLoading());
			})
			.catch(erro => console.log(JSON.stringify(erro)));
	};

	const listaDocumentacaoDcPreencher = tipoDocumentacaoList => {
		tipoDocumentacaoList.forEach(tipoDocumento => {
			...
			const docdb = _.find(documentos_db, doc => doc.tipoId === tipoDocumento.Id);
			if (docdb) {
	...
	const removeFile = () => {
		...
		setFieldTouched('documentacaoRemover', true);
	...
	const { submitCount, getFieldProps, setFieldValue, values, setFieldTouched } = formulario;
