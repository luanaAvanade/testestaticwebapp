...
export default function DadosSocios({
	...
	useEffect(
		() => {
			if (acao && acao !== null && tab === TAB_DADOS_SOCIOS) {
				switch (acao) {
		...
		setValues({
			...
			capitalSocialTotal:
				dados.CapitalSocialTotalSociedade === ''
					? ''
					: moeda(dados.CapitalSocialTotalSociedade * 100),
	...
	const insereDadosSocios = async empresaSave => {
		const responseInsert = await EmpresaService.update(empresa.Id, empresaSave);
